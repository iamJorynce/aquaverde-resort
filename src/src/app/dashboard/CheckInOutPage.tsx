...
export default function CheckInOutPage() {
  ...
  const [billDetail, setBillDetail] = useState<{ group: BookingGroup; addons: any[] } | null>(null)

  // Checkout payment modal (works on a GROUP)
  const [checkoutModal, setCheckoutModal] = useState<{ group: BookingGroup; addons: any[] } | null>(null)
  ...
  async function viewBill(group: BookingGroup) {
    const { data: addons } = await supabase
      .from('booking_addons').select('*').eq('booking_id', group.primary.id).order('created_at')
    setBillDetail({ group, addons: addons ?? [] })
  }
  ...
  async function openCheckoutModal(group: BookingGroup) {
    ...
    const [{ data: addons }, { data: rentals }] = await Promise.all([
      supabase.from('booking_addons').select('*').eq('booking_id', group.primary.id).order('created_at'),
      supabase.from('equipment_rentals')
        .select('id, equipment_id, quantity, equipment(name)')
        .eq('booking_id', group.primary.id)
        .is('rental_end', null),
    ])
    ...
    if (rentals && rentals.length > 0) {
      ...
      setCheckoutModal({ group, addons: addons ?? [] })
  ...
  async function confirmEquipmentCheck() {
    ...
    const { data: updatedAddons } = await supabase.from('booking_addons').select('*').eq('booking_id', refreshedPrimary.id).order('created_at')
    setCheckoutModal({ group: refreshedGroup, addons: updatedAddons ?? [] })
  ...
  async function confirmCheckout() {
    ...
    const { group, addons } = checkoutModal
    ...
    printReceipt({
      ...
      guestName,
      lineItems: [...roomLines, ...addonLines],
