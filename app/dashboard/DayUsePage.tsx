...
export default function DayUsePage() {
  ...
  async function load() {
    ...
    // Check for active shift (cashier/front desk only)
      ...
      {hasActiveShift === false && (
        <div className="mb-4 bg-red-50 border border-red-200 rounded-xl px-4 py-3 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-red-500">You must open a shift in the <strong>Remittance</strong> section before recording entries.</div>
          </div>
        </div>
      )}
      {success && (
